...
storage bind). A dedicated file rather than `services.yaml`: native daemons do
not fit the compose-shaped `services` record. Type-specific, not free-form
passthrough."""
...
class SftpUserInventory(BaseModel):
    ...
    name: str
    # Authorized SSH *public* keys (>=1), verbatim. Public keys are not
    # secret, so they ride as plain inventory data instead of sops references
    # (T-078).
    ...
    def _normalize_keys(cls, value: list[str]) -> list[str]:
        ...
        for key in normalized:
            # Catch a pasted private key or multi-line blob before it lands
            # in a rendered module.
